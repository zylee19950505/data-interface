...
//                    break;
    ...
    private void crtImpOrderData(List<PackageXml> packageXmlList, String refileName) throws Exception {
        ...
        for (PackageXml packageXml : packageXmlList) {
            ...
            receiverId = envelopInfo.getReceiverId();
            icCard = envelopInfo.getIcCard();
            status = envelopInfo.getStatus();

            Enterprise enterprise = this.dockingMapper.queryEntInfoById(senderId);
            ...
            for (DataInfo dataInfo : dataInfoList) {
                ...
                int count = this.dockingMapper.findRepeatOrder(dataHead.getDataId(), dataHead.getOrderNo());
                if (count > 0) {
                    this.logger.debug(String.format("》》》》》》》》》》》对接订单报文重复数据:[guid：%s,orderNo：%s]", dataHead.getDataId(), dataHead.getOrderNo()));
                    continue;
                }
                ...
                impOrderHead.setOrder_Type(dataHead.getOrderType());//电子订单类型：I进口
                impOrderHead.setBuyer_Id_Type("1");//订购人证件类型
                impOrderHead.setCurrency(dataHead.getCurrency());//币制
                impOrderHead.setData_status(StatusCode.BSYDR);//数据状态
                impOrderHead.setNote(dataHead.getNote());
                ...
                impOrderHead.setBws_no(dataHead.getBwsNo());
                impOrderHead.setCustoms_code(dataHead.getCustomsCode());

                impOrderHead.setGuid(dataHead.getDataId());
                ...
                impOrderHead.setEbc_Code(dataHead.getEbcCode());
                impOrderHead.setEbc_Name(dataHead.getEbcName());
                ...
                for (DataBody dataBody : dataBodyList) {
                    ...
                    BwlListType bwlListType = this.bondOrderImpMapper.queryBwlListTypeByItemNo(dataBody.getItemNo());

                    impOrderBody = new ImpOrderBody();
                    impOrderBody.setHead_guid(dataHead.getDataId());
                    ...
                    impOrderBody.setCountry(dataBody.getCountry());//账册商品国别码
                    impOrderBody.setCurrency(dataBody.getCurrency());//币制
...
//                this.insertOrderNo(impOrderHead);
                this.logger.debug(String.format("》》》》》》》》》》》对接订单报文入库成功:[guid：%s,orderNo：%s]", dataHead.getDataId(), dataHead.getOrderNo()));
    ...
    private void crtBondInvtData(List<PackageXml> packageXmlList, String refileName) throws Exception {
        ...
        for (PackageXml packageXml : packageXmlList) {
            ...
            for (DataInfo dataInfo : dataInfoList) {
                ...
                int count = this.dockingMapper.findRepeatBondInvt(dataHead.getDataId());
                if (count > 0) {
                    this.logger.debug(String.format("》》》》》》》》》》》对接入区核注清单报文重复数据:[EtpsInnerInvtNo：%s]", dataHead.getDataId()));
                    continue;
                }
                ...
                for (DataBody dataBody : dataBodyList) {
                    ...
                    bondInvtDt.setHead_etps_inner_invt_no(dataHead.getDataId());
                ...
                bondInvtBsc.setEtps_inner_invt_no(dataHead.getDataId());
                ...
                bondInvtBsc.setBizop_etps_sccd(dataHead.getBizopEtpsSccd());
                bondInvtBsc.setBizop_etpsno(dataHead.getBizopEtpsno());
                bondInvtBsc.setBizop_etps_nm(dataHead.getBizopEtpsNm());
                ...
                bondInvtBsc.setRvsngd_etps_sccd(dataHead.getRvsngdEtpsSccd());
                bondInvtBsc.setRcvgd_etpsno(dataHead.getRcvgdEtpsno());
                bondInvtBsc.setRcvgd_etps_nm(dataHead.getRcvgdEtpsNm());
                ...
                bondInvtBsc.setMtpck_endprd_markcd(dataHead.getMtpckEndprdMarkcd());
                bondInvtBsc.setSupv_modecd(dataHead.getSupvModecd());
                bondInvtBsc.setDclcus_flag(dataHead.getDclcusFlag());
                bondInvtBsc.setBond_invt_typecd(dataHead.getInvtType());
                ...
                bondInvtBsc.setStatus(StatusCode.BSYDR);
                bondInvtBsc.setBusiness_type(SystemConstants.T_BOND_INVT);
                bondInvtBsc.setRmk(dataHead.getRmk());

                this.dockingMapper.insertBondInvtBsc(bondInvtBsc);

                this.logger.debug(String.format("》》》》》》》》》》》对接入区核注清单报文入库成功:[EtpsInnerInvtNo：%s]", dataHead.getDataId()));
            }
