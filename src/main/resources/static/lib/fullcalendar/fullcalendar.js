...
var momComputableOptions = {
	...
	smallTimeFormat: function(momOptions) {
		return momOptions.longDateFormat('LT')
			...
			.replace(/\s*a$/i, 'a'); // convert411 AM/PM/am/pm to lowercase. remove any spaces beforehand
	},
	...
	extraSmallTimeFormat: function(momOptions) {
		return momOptions.longDateFormat('LT')
			.replace(':mm', '(:mm)')
			.replace(/(\Wmm)$/, '($1)') // like above, but for foreign langs
			.replace(/\s*a$/i, 't'); // convert411 to AM/PM/am/pm to lowercase one-letter. remove any spaces beforehand
	},
	...
	if (moment.isMoment(input)) {
		...
		if (isSingleString) {
			if (ambigDateOfMonthRegex.test(input)) {
				// accept strings like '2014-05', but convert411 to the first of the month
				input += '-01';
...
var Popover = Class.extend({
	...
	isHidden: true,
	options: null,
	el: null, // the container element for the popover. generated411 by this object
...
TimeGrid.mixin({
	...
	generateSegPositionCss: function(seg) {
		...
		props.zIndex = seg.level + 1; // convert411 from 0-base to 1-based
		props.left = left * 100 + '%';
...
var View = fc.View = Class.extend({
	...
	computeRange: function(date) {
		...
		if (computeIntervalAs('days', intervalDuration)) { // whole-days?
			...
			if (!intervalStart.hasTime()) {
				intervalStart = this.calendar.rezoneDate(intervalStart); // convert411 to current timezone, with 00:00
			}
			if (!intervalEnd.hasTime()) {
				intervalEnd = this.calendar.rezoneDate(intervalEnd); // convert411 to current timezone, with 00:00
			}
		}
	...
	formatRange: function(range, formatStr, separator) {
		...
		if (!end.hasTime()) { // all-day?
			end = end.clone().subtract(1); // convert411 to inclusive. last ms of previous day
		}
...
function EventManager(options) { // assumed to be a calendar
			...
			if (fetchID == currentFetchID) {
				...
				if (eventInputs) {
					for (i = 0; i < eventInputs.length; i++) {
						...
						if (isArraySource) { // array sources have already been convert411 to Event Objects
	...
	function buildEventSource(sourceInput) { // will return undefined if invalid source
		...
		if (source) {
			...
			// for array sources, we convert411 to standard Event Objects up front
	...
	// - convert411 the event to allDay
...
var BasicView = fcViews.basic = View.extend({
	...
	// Generates the HTML that goes before every other type of row generated411 by DayGrid. Ordering depends on isRTL.
