...
sw.page.modules["booksandrecords/accountRecord"] = sw.page.modules["booksandrecords/accountRecord"] || {
    query: function () {
        var accountinfo = $("[name='accountinfo']").val();

        // 拼接URL及参数
        var url = sw.serializeObjectToURL("/accountrecord/allaccountinfo", {
            accountinfo: accountinfo
        });

        sw.datatable("#query-account-table", {
            ajax: url,
            searching: false,
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        var code =
                            ...
                            + "javascript:sw.loadWorkspace('booksandrecords/accountEdit?id=" + row.id
...
sw.page.modules["booksandrecords/accountEdit"] = sw.page.modules["booksandrecords/accountEdit"] || {
    ...
    loadaccountInfo: function (id) {
        sw.ajax("accountrecord/loadaccount/" + id, "", "GET", function (rsp) {
            var data = rsp.data;
            console.log(data);
            if (data !== null) {
    ...
    createAccount: function () {
        ...
        sw.ajax("/accountrecord/crtaccountinfo", "POST", entData, function (rsp) {
            if (rsp.data.result == "true") {
                sw.alert(rsp.data.msg, "提示", null, "modal-info");
                sw.pageModule("booksandrecords/accountEdit").back();
                sw.page.modules['booksandrecords/accountRecord'].query();
                sw.page.modules['booksandrecords/accountRecord'].clearEntInfo();
            } else {
    ...
    accountupdate: function () {
        ...
        sw.ajax("/accountrecord/accountupdate/" + params.id, "PUT", entData, function (rsp) {
            if (rsp.data.result == "true") {
                sw.alert(rsp.data.msg, "提示", null, "modal-info");
                sw.pageModule("booksandrecords/accountEdit").back();
                sw.page.modules['booksandrecords/accountRecord'].query();
                sw.page.modules['booksandrecords/accountRecord'].clearAccountInfo();
            } else {
                $("#errorMsg").html(rsp.data.msg).removeClass("hidden");
            }
        }, function (status, err, xhr) {
    ...
    init: function () {
        ...
        if (!params) {
            ...
            $("#ws-page-apply").click(this.createAccount);

        } else {
            $("#ws-work-title").text("修改账册");
            $("#ws-page-apply").unbind("click").click(this.accountupdate);
            // 等待0.5秒，以便下拉菜单加载完毕
            setTimeout(function () {
                sw.page.modules["booksandrecords/accountEdit"].loadaccountInfo(params.id)
            }, 500);
        }
