 ...
 * 订单申报
 */
sw.page.modules["bondordermanage/bondOrderDecl"] = sw.page.modules["bondordermanage/bondOrderDecl"] || {
    // 订单申报列表查询
    query: function () {
        // 获取查询表单参数
        ...
        // 拼接URL及参数
        var url = sw.serializeObjectToURL("api/bondordermanage/queryOrderDeclare", {
    ...
    submitCustom: function () {
        ...
        sw.confirm("请确认分单总数无误，提交海关", "确认", function () {
            ...
            sw.ajax("api/bondordermanage/submitCustom", "POST", postData, function (rsp) {
                if (rsp.data.result == "true") {
                    sw.alert("提交海关成功", "提示", function () {
                    }, "modal-success");
                    $("#submitManifestBtn").prop("disabled", false);
                    sw.page.modules["bondordermanage/bondOrderDecl"].query();
                } else {
    ...
    orderXmlDownLoad: function () {
        ...
        sw.confirm("请确认分单总数无误，提交海关", "确认", function () {
            ...
            sw.ajax("api/bondordermanage/orderXmlDownload", "POST", postData, function (rsp) {
                ...
                if (str.substring(0, 1) == "1") {
                    ...
                    sw.page.modules["bondordermanage/bondOrderDecl"].query();
                    window.location.href = "/api/bondordermanage/downloadFile?type=" + str.substring(1);
                } else {
                    sw.alert(rsp.data.msg);
                }
                $.unblockUI();
